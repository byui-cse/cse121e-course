...
most_popular_combinations(Count,List)->
	{Keepers,Rest} = lists:split(Count,List),
	Keepers.
...
%%For this set of tests, expected results are defined first, then the unit tests are done.
...
cone_combinations_one_list_test_() ->
	...
	Top_scoop_list = [vanilla,chocolate,cherry_ripple],
	Bottom_scoop_list = [lemon,butterscotch,licorice_riple],
	%all possible combinations from either list for top and bottom scoops
...
%This list consists of 2-tuples of the form {integer(),{atom(),atom()}}.
	%The integer in the tuple is the number of purchases of the cone pair.
